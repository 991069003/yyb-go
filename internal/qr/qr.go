...
func NewClient(timeout time.Duration, tcpProxy string) *Client {
	return &Client{
		timeout:      timeout,
		tcpProxy:     tcpProxy,
...
// transport returns a custom *http.Transport that dials through the configured
// SOCKS5 proxy when one is set. Returns nil to use
// the default transport.
func (c *Client) transport() *http.Transport {
	if c.tcpProxy == "" {
		return nil
	}
	if _, err := protocol.ParseTCPProxy(c.tcpProxy); err != nil {
		return nil
	}
	...
	return &http.Transport{
		...
		DialContext: func(ctx context.Context, network, addr string) (net.Conn, error) {
			...
			return protocol.DialTCP(ctx, host, port, dialTimeout, c.tcpProxy, true)
		},
