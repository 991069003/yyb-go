...
func Open(path string) (*DB, error) {
	...
	if err = migrateAPITokensSecret(ctx, db); err != nil {
		_ = db.Close()
		return nil, err
	}
	out := &DB{sql: db}
...
func migrateAPITokensSecret(ctx context.Context, db *sql.DB) error {
	...
	var n int
	err = db.QueryRowContext(ctx, "SELECT count(*) FROM pragma_table_info('api_tokens') WHERE name='secret'").Scan(&n)
	if err != nil {
		return err
	}
	if n == 0 {
		_, err = db.ExecContext(ctx, "ALTER TABLE api_tokens ADD COLUMN secret TEXT")
		if err != nil {
			return err
		}
	}
...
// APIToken 表示一条 API Token 记录。secret 原文现在持久化在数据库中，
// 用于首页管理面板直接展示；secret_hash 仍用于鉴权。
...
// ListAPITokens 返回全部 Token（含明文 secret 与哈希）。
func (db *DB) ListAPITokens(ctx context.Context) ([]APIToken, error) {
	rows, err := db.sql.QueryContext(ctx,
		"SELECT id, name, secret, secret_hash, enabled, created_at, updated_at FROM api_tokens ORDER BY id")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (db *DB) GetAPIToken(ctx context.Context, id int64) (*APIToken, error) {
	row := db.sql.QueryRowContext(ctx,
		"SELECT id, name, secret, secret_hash, enabled, created_at, updated_at FROM api_tokens WHERE id=?", id)
	return scanAPIToken(row)
}

// FindAPITokenByHash 按密钥哈希查询（用于鉴权）。
func (db *DB) FindAPITokenByHash(ctx context.Context, hash string) (*APIToken, error) {
	row := db.sql.QueryRowContext(ctx,
		"SELECT id, name, secret, secret_hash, enabled, created_at, updated_at FROM api_tokens WHERE secret_hash=?", hash)
	return scanAPIToken(row)
}

// CountAPITokens 统计 Token 总数（用于首次播种判断）。
func (db *DB) CountAPITokens(ctx context.Context) (int64, error) {
	var n int64
	err := db.sql.QueryRowContext(ctx, "SELECT count(*) FROM api_tokens").Scan(&n)
	return n, err
}

// CreateAPIToken 写入一条新 Token（secret 为明文，secretHash 为 SHA-256 十六进制）。
func (db *DB) CreateAPIToken(ctx context.Context, name, secret, secretHash string) (*APIToken, error) {
	now := time.Now().Unix()
	if name == "" {
		name = "Token"
	}
	res, err := db.sql.ExecContext(ctx,
		"INSERT INTO api_tokens (name, secret, secret_hash, enabled, created_at, updated_at) VALUES(?,?,?,1,?,?)",
		name, secret, secretHash, now, now)
	if err != nil {
		return nil, err
	}
	...
	return db.GetAPIToken(ctx, id)
}

// UpdateAPIToken 修改名称与启用/禁用状态。
func (db *DB) UpdateAPIToken(ctx context.Context, id int64, name string, enabled bool) (*APIToken, error) {
	_, err := db.sql.ExecContext(ctx,
		"UPDATE api_tokens SET name=?, enabled=?, updated_at=? WHERE id=?",
		name, boolToInt(enabled), time.Now().Unix(), id)
	if err != nil {
		return nil, err
	}
	return db.GetAPIToken(ctx, id)
...
// UpdateAPITokenFull 同时修改名称、启用状态与密钥（secret/secretHash 为 nil 表示不改密钥）。
func (db *DB) UpdateAPITokenFull(ctx context.Context, id int64, name string, enabled bool, secret, secretHash *string) (*APIToken, error) {
	now := time.Now().Unix()
	if secretHash != nil {
		_, err := db.sql.ExecContext(ctx,
			"UPDATE api_tokens SET name=?, enabled=?, secret=?, secret_hash=?, updated_at=? WHERE id=?",
			name, boolToInt(enabled), *secret, *secretHash, now, id)
		if err != nil {
			return nil, err
		}
	} else {
...
// SetAPITokenSecret 重置密钥（更新明文与哈希）。
func (db *DB) SetAPITokenSecret(ctx context.Context, id int64, secret, secretHash string) error {
	_, err := db.sql.ExecContext(ctx,
		"UPDATE api_tokens SET secret=?, secret_hash=?, updated_at=? WHERE id=?", secret, secretHash, time.Now().Unix(), id)
	return err
}

// DeleteAPIToken 删除一条 Token（密钥随之失效）。
func (db *DB) DeleteAPIToken(ctx context.Context, id int64) error {
	_, err := db.sql.ExecContext(ctx, "DELETE FROM api_tokens WHERE id=?", id)
	return err
}

// SeedDefaultAPIToken 仅当 api_tokens 表为空时，把已有的全局 Token（rawSecret）
// 以「初始 Token」名义播种进表，保证历史接入方不受影响。若已存在记录但 secret
// 为空（老版本仅存 hash），则用 rawSecret 回填。
func (db *DB) SeedDefaultAPIToken(ctx context.Context, rawSecret, secretHash string) error {
	n, err := db.CountAPITokens(ctx)
	if err != nil {
		return err
	}
	if n == 0 {
		_, err = db.CreateAPIToken(ctx, "初始 Token", rawSecret, secretHash)
		return err
	}
	// 回填：把 secret_hash 匹配且 secret 为空的记录补上明文。
	_, _ = db.sql.ExecContext(ctx,
		"UPDATE api_tokens SET secret=? WHERE secret_hash=? AND (secret IS NULL OR secret='')",
		rawSecret, secretHash)
	return nil
}

func scanAPIToken(row accountScanner) (*APIToken, error) {
	var t APIToken
	var enabled int
	if err := row.Scan(&t.ID, &t.Name, &t.Secret, &t.SecretHash, &enabled, &t.CreatedAt, &t.UpdatedAt); err != nil {
		return nil, err
	}
	t.Enabled = enabled != 0
	return &t, nil
